...
The final part of the document covers the usual README content; requirements  for running locally, as well as the API description. 
# Approach
I wanted to use the exercise to demonstrate not just code, but design, architecture and project management. To do this I've approached the problem as if I were developing the tool into a full product. Focusing on what would go into an MVP, I've wireframed a UI, built a backend that focuses on extensible architecture and made a business argument for the infrastructure I've chosen. 

# Design
## UI
The first thing I designed was the UI. As this is a user facing tool, the UI and UX are key. Designing the UI first allows us to create a user-focused experience. The wireframe can be seen here: 
![](./img/listview.png)
![](./img/mapview.png)
...
I would build this as a SPA, the majority of components created using bootstrap and the map pulled from a library such as google-map-react, which is a react library built to interact with the Google Maps API.
## Backend
### API Design
The proposed UI design will require 3 endpoints `GET FilterInformation` to populate the sidebar, `GET Venues` to populate the list/map and `GET GoogleApiKey` to fetch the Google maps API key, stored in a KeyVault for security.

This design uses both `GET Venues` for the list and map view, which returns all data matching the filter. Loading all the data upfront allows users to switch between pages, views and venues without having to wait for data to be fetched from the backend each time. However, there is a chance that `GET Venues` will be returning too much data causing initial load times to be too long. In which case I would pivot to have `GET ListView`, `GET MapView` and `GET Venue`. `GET ListView` would paginate the data on the backend. `GET MapView` would return only location and name data in order to place the points on the map. `GET Venue` would be then called whenever a user clicked on a specific venue to get the rest of the information for the callout. Again, this data can be cached to allow for faster loading if a user is flicking back and forth between venues or pages.
...
## Infrastructure
Due to the nature of the application, I thought low running costs would be a priority when it came to choosing infrastructure. Because of this, I've decided to use serverless infrastructure in the form of Azure Functions which provides an incredibly cheap and straightforward way to host a web app, whilst also providing table storage for the data.
...
- Visual Studio 2019
## Debugging
To Run the program, simply run the BeerQuest.Functions project from within Visual Studio. The functions SDK will automatically start up the storage emulator. A triggered function has been built to create and populate the table on startup for the purpose of this exercise.
## Api Definition
### GetVenues 
#### Request
```
...
NameSearch|string|Filters out any venues that whos names don't contain this string|
